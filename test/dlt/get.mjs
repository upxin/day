...
singleTickets.forEach((ticket) => {
  ...
  if (commonCount === 5 || commonCount === 4 || commonCount === 3) {
...
async function exportGroupedTickets() {
  for (const [count, tickets] of Object.entries(groupedTickets)) {
    let dataString = `const allSingleTicketsWith${count}Matches = [\n`;
    ...
    try {
      ...
      console.log(
        `所有与目标数据有 ${count} 个相同数字的单式组合已导出到 allSingleTicketsWith${count}Matches.js 文件中`
      );
    } catch (error) {
      console.error(
        `写入 allSingleTicketsWith${count}Matches.js 文件时出错:`,
        error
      );
    }
  }
